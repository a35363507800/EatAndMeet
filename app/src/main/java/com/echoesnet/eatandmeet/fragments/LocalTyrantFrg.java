...
public class LocalTyrantFrg extends BaseFragment
{

    private final String TAG = LocalTyrantFrg.class.getSimpleName();
    @BindView(R.id.web_date_wish_list)
    ProgressBridgeWebView mWebView;
    Unbinder unbinder;
    private final int START_FOR_MY_USERINFO = 101;
    private final int RESULT_FROM_USERINFO = 20;
    ...
    private void afterView()
    {
        ...
        mWebView.loadUrl(EAMCheckCacheFiles.tryH5CacheFile(NetInterfaceConstant.data_local_tyrant));
        Logger.t(TAG).d("isHardwareAccelerated：》》" + mWebView.isHardwareAccelerated());
        Logger.t(TAG).d("NetInterfaceConstant.data_local_tyrant》》" + NetInterfaceConstant.data_local_tyrant);
        ...
        mWebView.registerHandler("openUserInfo", new BridgeHandler()
        {
            ...
            public void handler(String data, CallBackFunction function)
            {
                ...
                try
                {
                    ...
                    intent.putExtra("open-from", "LocalTyrantFrg");
                    mActivity.startActivityForResult(intent, START_FOR_MY_USERINFO);
        ...
        switch (requestCode)
        {
            case START_FOR_MY_USERINFO:
                if (resultCode == RESULT_FROM_USERINFO)
                {
                    ...
                    {
                        reLoadWebView(uid);
                        Logger.t(TAG).d("LocalTyrantFrg执行H5方法");
                    }
