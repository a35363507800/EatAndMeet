...
public class FaceFanFrg extends BaseFragment
{
    private final String TAG = FaceFanFrg.class.getSimpleName();
    ...
    private void afterView()
    {
        ...
        mWebView.loadUrl(EAMCheckCacheFiles.tryH5CacheFile(NetInterfaceConstant.data_face_fan));
        ...
        mWebView.registerHandler("openUserInfo", new BridgeHandler()
        {
            ...
            public void handler(String data, CallBackFunction function)
            {
                ...
                try
                {
                    ...
                    intent.putExtra("open-from", "FaceFanFrg");
                    startActivityForResult(intent, START_FOR_MY_USERINFO);
        ...
        switch (requestCode)
        {
            case START_FOR_MY_USERINFO:
                if (resultCode == RESULT_FROM_USERINFO)
                {
                    ...
                    {
                        reLoadWebView(uid);
                        Logger.t(TAG).d("FaceFanFrg执行H5方法");
                    }
