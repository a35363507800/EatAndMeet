...
public class EaseNotifier
{
    ...
    protected Set<String> fromUsers = new HashSet<String>();
    ...
    {
        if (ChatCommonUtils.isSilentMessage(message))
        {
            EMLog.d(TAG, "return了---》" + 1);
            return;
        }
        ...
        {
            EMLog.d(TAG, "easeUi >>： " + 1);
            sendNotification(message, false);
        }
        else
        {
            EMLog.d(TAG, "easeUi >>： " + 2);
    ...
    {
        if (ChatCommonUtils.isSilentMessage(messages.get(messages.size() - 1)))
        {
            return;
        }
        EaseUI.EaseSettingsProvider settingsProvider = EaseUI.getInstance().getSettingsProvider();
    ...
    {
        String username = message.getStringAttribute(EamConstant.EAM_CHAT_ATTR_NICKNAME, "游客");
        try
        {
            ...
            eUser.setNickName(message.getStringAttribute(EamConstant.EAM_CHAT_ATTR_NICKNAME, "游客"));
            eUser.setuId(message.getStringAttribute(EamConstant.EAM_CHAT_ATTR_UID, "123456"));
            eUser.setAvatar(message.getStringAttribute(EamConstant.EAM_CHAT_ATTR_HEADIMAGE, ""));
            eUser.setId(message.getStringAttribute(EamConstant.EAM_CHAT_ATTR_ID, "123456"));
            eUser.setLevel(message.getStringAttribute(EamConstant.EAM_CHAT_ATTR_LEVEL, "15"));
            eUser.setAge(message.getStringAttribute(EamConstant.EAM_CHAT_ATTR_AGE, "15"));
            eUser.setIsVuser(message.getStringAttribute(EamConstant.EAM_CHAT_ATTR_VUSER, "0"));

            msgIntent.putExtra(Constant.EXTRA_TO_EASEUSER, eUser);
            ...
            if (numIncrease)
            {
                ...
                //不让判断是否前后台 都让推送，消息增加一条
//                if (!isForeground)
//                {
                notificationNum++;
                System.out.println("消息数目：" + notificationNum);
                fromUsers.add(message.getFrom());
//                }
            }

            int fromUsersNum = fromUsers.size();
            System.out.println("easeUi 消息数目： " + Integer.toString(notificationNum));
            ...
            Logger.t(TAG).d("推送的值" + contentTitle + "/" + notifyText + "/" + summaryBody + "/" + isForeground + "/");
            ...
            if (Build.MANUFACTURER.equalsIgnoreCase("Xiaomi"))
            {
                try
                {
                    ...
                    method.invoke(extraNotification, EamApplication.getInstance().getBadgerCount());
                } catch (Exception e)
                {
