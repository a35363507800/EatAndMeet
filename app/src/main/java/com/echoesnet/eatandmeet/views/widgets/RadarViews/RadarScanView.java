package com.echoesnet.eatandmeet.views.widgets.RadarViews;
...
public class RadarScanView extends View
{
    ...
    private boolean hasDetachedFromWindow = false;

    private RadarScanViewHandler handler = new RadarScanViewHandler(this);
    ...
    {
        hasDetachedFromWindow = true;
        super.onDetachedFromWindow();
    }
    private static class RadarScanViewHandler extends Handler{
        private WeakReference<RadarScanView> mViewRef;

        public RadarScanViewHandler(RadarScanView radarScanView)
        {
            mViewRef = new WeakReference<RadarScanView>(radarScanView);
        }
        ...
        public void handleMessage(Message msg)
        {
            ...
            RadarScanView radarScanView = mViewRef.get();
            if (radarScanView.hasDetachedFromWindow)
                return;
            radarScanView.start += 2;
            radarScanView.matrix = new Matrix();
            radarScanView.matrix.postRotate(radarScanView.start, radarScanView.centerX, radarScanView.centerY);
