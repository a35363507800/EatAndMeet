...
public class CameraInterface implements Camera.PreviewCallback {
    ...
    private IconTextView mSwitchView;

    public void setSwitchView(IconTextView mSwitchView) {
...
//                mCamera.stopPreview();
    ...
    void takePicture(final String showType, final TakePictureCallback callback) {
        if (mCamera == null) {
            return;
        }
        switch (cameraAngle) {
...
//
        ...
        mCamera.takePicture(null, null, new Camera.PictureCallback() {
            ...
            public void onPictureTaken(byte[] data, Camera camera) {
                ...
                if ("2".equals(showType)||"4".equals(showType))
    ...
    //启动录像
    void startRecord(String showType,Surface surface, ErrorCallback callback) {
        mCamera.setPreviewCallback(null);
        ...
        } else if ("2".equals(showType) || "4".equals(showType))
        {
            if (SELECTED_CAMERA == CAMERA_POST_POSITION)
            {
                matrix.setRotate(nowAngle);
...
//        }
    ...
    //停止录像
    void stopRecord(boolean isShort,String showType, StopRecordCallback callback) {
        if (!isRecorder) {
            return;
        }
        if (mediaRecorder != null) {
    ...
    public void handleFocus(final Context context, final float x, final float y, final FocusCallback callback) {
        ...
        try {
            ...
            Logger.t(TAG).e("autoFocus failer");
        }
    }
    ...
    private static Rect calculateTapArea(float x, float y, float coefficient, Context context) {
        ...
        int centerX = (int) (x / CommonUtils.getScreenHeight(context) * 2000 - 1000);
        int centerY = (int) (y / CommonUtils.getScreenWidth(context) * 2000 - 1000);
