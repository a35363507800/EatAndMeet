    ...
    public void init(Context context, AttributeSet attrs)
    {
        ...
        if (attrs != null)
        {
            ...
            if (a.hasValue(R.styleable.MagicTextView_mtvStrokeColor))
            {
                ...
                this.setStroke(CommonUtils.dp2px(mContext,(int) strokeWidth), strokeColor,
                        strokeJoin,CommonUtils.dp2px(mContext, (int) strokeMiter));
            }
        }
    }
...
//          paint.setTextAlign(Paint.Align.CENTER);
    ...
    public void startShow(final float maxScale, final float textSize)
    {
        Animation a = null;
        clearAnimation();
        //final float startScaleFactor = 1.0F;
        a = new Animation()
        {
            @Override
            protected void applyTransformation(float interpolatedTime, Transformation t)
            {
                Logger.t("TEXT>").d("差值》" + interpolatedTime);
                setTextSize(TypedValue.COMPLEX_UNIT_SP, (1 + maxScale * interpolatedTime) * textSize);
            }
        };
        a.setDuration(1000);
