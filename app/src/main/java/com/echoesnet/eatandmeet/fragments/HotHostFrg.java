...
public class HotHostFrg extends BaseFragment
{
    private final String TAG = HotHostFrg.class.getSimpleName();
    ...
    private void afterView()
    {
        ...
        mWebView.loadUrl(EAMCheckCacheFiles.tryH5CacheFile(NetInterfaceConstant.data_hot_host));
        ...
        mWebView.registerHandler("openUserInfo", new BridgeHandler()
        {
            ...
            public void handler(String data, CallBackFunction function)
            {
                ...
                try
                {
                    ...
                    intent.putExtra("open-from", "HotHostFrg");
        ...
        switch (requestCode)
        {
            case START_FOR_MY_USERINFO:
                if (resultCode == RESULT_FROM_USERINFO)
                {
                    ...
                    {
                        reLoadWebView(uid);
                        Logger.t(TAG).d("HotHostFrg执行H5方法");
                    }
